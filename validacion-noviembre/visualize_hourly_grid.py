...
Based on physical luxmeter measurement grid (7x9 = 63 sensors).
"""
...
# Validation grid parameters (from generate_sensor_grid.py)
# Room dimensions
...
def create_hourly_grid_figure(illuminance_data, x_coords, y_coords, date_str, output_file=None):
    ...
    Layout: 3 rows x 3 columns for hours 9, 10, 11, 12, 13, 14, 15, 16, 17
    """
    ...
    fig.suptitle(f'Validation Grid Hourly Illuminance - {date_str}\n'
                 f'Sensor Grid: {NX}x{NY} = {NX*NY} points, Spacing: {SPACING}m, Height: {WORK_PLANE_Z}m',
...
def main():
    ...
    parser.add_argument('--date', type=str, required=True,
                       help='Date in YYYY-MM-DD format (e.g., 2024-11-20)')
    ...
    try:
        ...
        print(f"Data shape: {illuminance_data.shape} (timesteps x sensors)")
